...
function PasswordStrength({ value }: PasswordStrengthProps) {
  ...
  const checks = requirements.map((requirement, index) => (
    <PasswordRequirement key={index} label={requirement.label} meets={requirement.re.test(inputValue)} />
  ));
  const bars = Array(4)
    .fill(0)
    .map((_, index) => (
      <Progress
        ...
        value={
          inputValue.length > 0 && index === 0 ? 100 : strength >= ((index + 1) / 4) * 100 ? 100 : 0
        }
  ...
  return (
    <div>
      ...
      <PasswordRequirement label="Has at least 6 characters" meets={inputValue.length > 5} />
      {checks}
    </div>
  );
}
